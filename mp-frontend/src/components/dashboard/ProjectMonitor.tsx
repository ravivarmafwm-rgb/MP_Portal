      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            MPLADS & state schemes in execution
          </p>
        </div>
        {list.length > 0 && (
      ...
      {list.length === 0 ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {list.slice(0, 4).map((p, i) => {
            const tone =
              statusTone[p.status] ?? "bg-muted text-muted-foreground";
            ...
            return (
              <motion.div
                ...
                className={cn(
                  "group relative overflow-hidden rounded-xl border border-border/70 bg-card p-4 transition-all hover:shadow-elevated",
                  delayed && "border-warning/40",
                )}
              >
                {delayed && (
