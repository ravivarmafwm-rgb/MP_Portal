...
interface UrgentItem { title: string; meta: string; severity: Severity; }
      ...
      <div className="mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">Triaged by impact and SLA</p>
          </div>
        </div>
        {items.length > 0 && (
          <Badge variant="secondary" className="bg-destructive/10 text-destructive">{items.length} items</Badge>
        )}
      </div>
      {items.length === 0 ? (
        <EmptyState icon={AlertTriangle} title="Nothing urgent" description="You're all caught up!" />
      ) : (
        ...
        return (
          ...
          >
            <div className={cn("grid h-8 w-8 shrink-0 place-items-center rounded-md border", sevTone[sev])}>
            ...
            <Badge variant="secondary" className={cn("border shrink-0", sevTone[sev])}>{sev}</Badge>
          </motion.div>
        );
      })}
        </div>
