...
function MpladsPage() {
  ...
  const mpladsKpis = [
    {
      l: "Budget Allocated",
      v: `₹${(totalBudget / 10000000).toFixed(1)}Cr`,
      icon: IndianRupee,
      tone: "bg-primary/10 text-primary",
    },
    {
      l: "Budget Utilized",
      v: `₹${(totalSpent / 10000000).toFixed(1)}Cr`,
      icon: Wallet,
      tone: "bg-success/10 text-success",
    },
    {
      l: "Pending",
      v: `₹${((totalBudget - totalSpent) / 10000000).toFixed(1)}Cr`,
      icon: Clock,
        ...
        actions={
          <>
            <Button variant="outline" size="sm" className="gap-1.5">
              <Download className="h-4 w-4" /> MPLADS Statement
            </Button>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            <h3 className="font-display text-base font-bold">
              Allocation Analytics · By Category
            </h3>
        ...
        <Card className="p-5">
          <div className="flex items-center justify-between">
            ...
            <Button asChild variant="ghost" size="sm">
              <Link to="/projects/development">View all</Link>
            </Button>
          </div>
          {isLoading ? (
              ...
              {projects.map((p: Record<string, unknown>) => (
                ...
                >
                  <div className="flex items-center gap-2 text-[10px]">
                    ...
                    >
                      <Landmark className="mr-1 h-3 w-3" /> Project
                    </Badge>
                  </div>
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    {String(p.location ?? "—")} · ₹
                    {(Number(p.sanctioned_amount ?? 0) / 100000).toFixed(1)}L
                  </div>
