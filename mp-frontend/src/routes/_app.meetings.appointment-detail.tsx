...
import { fetchAppointment, updateAppointment } from "@/lib/api";
...
function AppointmentDetailPage() {
  ...
  const { mutate: doUpdate, isPending: updating } = useMutation({
    mutationFn: ({ status }: { status: string }) =>
      updateAppointment(String(appointment?.id ?? ""), { status }),
    onSuccess: () => {
  ...
  const timeline = [
    ...
    },
    {
      ...
      done: ["confirmed", "completed", "cancelled"].includes(String(a.status)),
      icon: Clock,
    },
    {
      step: "Confirmed",
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3 mt-1 text-sm text-muted-foreground">
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    Requested: {String(a.requested_date ?? "").substring(0, 10)}
                  </span>
                </div>
              </div>
            </div>
                ...
                <Card className="p-5">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <p className="text-sm rounded-lg bg-muted/40 p-3">
                      {String(a.meeting_outcome ?? "Not yet completed")}
                    </p>
                    <div className="text-muted-foreground mt-2">
                      Action Items
                    </div>
                    <p className="text-sm rounded-lg bg-muted/40 p-3">
                      {String(a.action_items ?? "—")}
                    </p>
                    ...
                    <div className="flex items-center gap-2">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star
                          ...
                          className={cn(
                            "h-6 w-6",
                            i < Number(a.satisfaction_rating ?? 0)
                      ...
                      <span className="text-sm font-semibold ml-1">
                        {Number(a.satisfaction_rating)}/5
                      </span>
                    </div>
                    {a.citizen_feedback && (
                      <p className="text-sm text-muted-foreground mt-2">
                        {String(a.citizen_feedback)}
                      </p>
                    )}
                  </Card>
                )}
          ...
          <TabsContent value="schemes">
            <Card className="p-6">
              <h4 className="font-semibold mb-4 flex items-center gap-2">
                ...
                Related Scheme Applications
              </h4>
              {a.scheme_application_id ? (
                <div className="rounded-lg border border-border/60 p-4">
                  <p className="text-sm font-medium">
                    Linked scheme application found
                  </p>
                  <Button asChild variant="outline" size="sm" className="mt-2">
                    <Link to="/schemes/application-detail">
                      View Application
                    </Link>
                  </Button>
                </div>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-3">
                ...
                {!a.meeting_outcome && !a.action_items && (
                  <div className="py-8 text-center text-sm text-muted-foreground">
                    No notes recorded yet. Notes will appear after the meeting
                    is conducted.
                  </div>
                )}
              </div>
