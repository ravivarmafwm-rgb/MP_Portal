...
const trendFallback = [
  { d: "Mon", filed: 28, resolved: 22 },
  { d: "Tue", filed: 35, resolved: 30 },
  { d: "Wed", filed: 42, resolved: 33 },
  { d: "Thu", filed: 38, resolved: 40 },
  { d: "Fri", filed: 51, resolved: 45 },
  { d: "Sat", filed: 33, resolved: 38 },
      ...
      <div className="space-y-6 p-4 md:p-8">
              ...
              {(categories ?? [])
                ...
                .map((c: Record<string, unknown>, i: number) => {
                  const maxCount = Math.max(
                    ...(categories ?? []).map((x: Record<string, unknown>) =>
                      Number(x.grievances_count ?? 0),
                    ),
                  );
                  const pct =
                    maxCount > 0
                      ? Math.round(
                          (Number(c.grievances_count ?? 0) / maxCount) * 100,
                        )
                      : 0;
                  return (
        ...
        <div className="flex justify-end">
          <Button asChild variant="outline">
            <Link to="/grievances/list">View All Grievances →</Link>
          </Button>
        </div>
      </div>
    </>
