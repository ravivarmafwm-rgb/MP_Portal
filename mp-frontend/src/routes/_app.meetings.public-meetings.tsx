...
import {
  Building2, Calendar, MapPin, Users, Plus, Loader2,
  Clock, CheckCircle2, ArrowUpRight, Mic,
} from "lucide-react";
...
function PublicMeetingsPage() {
  ...
  const [form, setForm] = useState({
    title: "", description: "", meeting_type: "town_hall", venue: "",
    meeting_date: "", start_time: "10:00", expected_attendance: 100, chief_guest: "",
  ...
  const upcoming = meetings.filter((m: Record<string, unknown>) => m.status === "scheduled").length;
  const completedCount = meetings.filter((m: Record<string, unknown>) => m.status === "completed").length;
  const totalAttendance = meetings.reduce((acc: number, m: Record<string, unknown>) => acc + Number(m.actual_attendance ?? 0), 0);
    ...
    <>
      ...
      <div className="space-y-6 p-4 md:p-8">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            { label: "Total Meetings",  value: meetings.length, tone: "text-foreground" },
            { label: "Upcoming",        value: upcoming,         tone: "text-primary"    },
            { label: "Completed",       value: completedCount,   tone: "text-success"    },
            { label: "Total Attendance",value: totalAttendance,  tone: "text-info"       },
          ].map((s) => (
            <Card key={s.label} className="p-4 text-center">
              <div className={`font-display text-2xl font-bold tabular-nums ${s.tone}`}>{s.value.toLocaleString()}</div>
        ...
        <div className="flex flex-wrap gap-2">
          {["all", "town_hall", "community_meeting", "department_review", "awareness_program"].map((t) => (
            <Button key={t} size="sm" variant={typeFilter === t ? "default" : "outline"}
              onClick={() => setTypeFilter(t)} className="text-xs capitalize">
              {t.replace(/_/g, " ")}
            </Button>
          ))}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {meetings.map((m: Record<string, unknown>, i: number) => {
              const isPast = m.status === "completed";
              return (
                <motion.div key={String(m.id)} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                  <Card className="p-5 flex flex-col gap-3 hover:-translate-y-0.5 hover:shadow-elevated transition-all">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <Badge variant="secondary" className={cn("text-[10px] mb-1", typeTone[String(m.meeting_type ?? "town_hall")])}>
                          {String(m.meeting_type ?? "").replace(/_/g, " ")}
                        </Badge>
                        <h3 className="font-semibold text-sm leading-tight">{String(m.title ?? "")}</h3>
                      </div>
                      <Badge variant="secondary" className={cn("shrink-0 text-[10px]", statusTone[String(m.status ?? "scheduled")])}>
                        {String(m.status ?? "")}
                      </Badge>
                    </div>

                    <div className="space-y-1.5 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1.5"><MapPin className="h-3.5 w-3.5 shrink-0" />{String(m.venue ?? "")}</div>
                      <div className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5 shrink-0" />{String(m.meeting_date ?? "").substring(0, 10)} · {String(m.start_time ?? "10:00")}</div>
                      {m.chief_guest && (
                        <div className="flex items-center gap-1.5"><Mic className="h-3.5 w-3.5 shrink-0" />{String(m.chief_guest)}</div>
                      )}
                    </div>

                    <div className="grid grid-cols-2 gap-2 rounded-lg bg-muted/40 p-3 text-center text-xs">
                      <div>
                        <div className="font-bold text-base tabular-nums">{Number(m.expected_attendance ?? 0)}</div>
        ...
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid gap-4">
            <div className="space-y-1.5">
              ...
              <Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. Town Hall — Water Supply Review" />
            </div>
            <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label>Venue *</Label>
              <Input value={form.venue} onChange={e => setForm(f => ({ ...f, venue: e.target.value }))} placeholder="Community Hall, Village name…" />
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="space-y-1.5">
                ...
                <Input type="number" value={form.expected_attendance} onChange={e => setForm(f => ({ ...f, expected_attendance: Number(e.target.value) }))} />
              </div>
              <div className="space-y-1.5">
                <Label>Chief Guest</Label>
                <Input value={form.chief_guest} onChange={e => setForm(f => ({ ...f, chief_guest: e.target.value }))} placeholder="Hon. MP Ravi Varma" />
              </div>
            </div>
            <div className="space-y-1.5">
          ...
          <DialogFooter>
            ...
            <Button disabled={creating || !form.title || !form.venue || !form.meeting_date}
              onClick={() => doCreate({ ...form, start_time: form.start_time + ":00" } as Record<string, unknown>)}>
              {creating ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Scheduling…</> : "Schedule Meeting"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
