    ...
    <>
      ...
      <div className="space-y-6 p-4 md:p-8">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {[
            { label: "Avg Satisfaction",  value: `${avgSatisfaction}/5`,  icon: Star,          tone: "bg-warning/15 text-warning",     hint: "Based on completed appointments" },
            { label: "Follow-up Pending", value: followUpPending,          icon: AlertCircle,   tone: "bg-destructive/10 text-destructive", hint: "Require follow-up action" },
            { label: "Engaged Villages",  value: byVillage.length,         icon: MapPin,        tone: "bg-success/10 text-success",     hint: "Villages with appointments" },
          ].map((s, i) => (
          ...
          {isLoading ? <Skeleton className="h-56" /> : (
            <ResponsiveContainer width="100%" height={220}>
              <LineChart data={monthlyTrend}>
                ...
                <Line type="monotone" dataKey="public_meetings" stroke="hsl(var(--info))" strokeWidth={2} dot={false} name="Public Meetings" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </Card>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card className="p-6">
            ...
            {isLoading ? <Skeleton className="h-48" /> : (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
                  <Pie data={byCategory} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={75} label={({ name, percent }) => `${name} ${(percent * 100).toFixed(0)}%`}>
          ...
          <Card className="p-6">
            <h3 className="text-h3 font-bold mb-4">Citizen Satisfaction Ratings</h3>
            {isLoading ? <Skeleton className="h-48" /> : (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={satisfaction}>
                  ...
                  <YAxis tick={{ fontSize: 11 }} />
                  <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                  <Bar dataKey="count" fill="hsl(var(--warning))" radius={[4, 4, 0, 0]} name="Count" />
                </BarChart>
              </ResponsiveContainer>
            )}
          </Card>
            ...
            {isLoading ? <Skeleton className="h-52" /> : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={byVillage} layout="vertical">
                  ...
                  <YAxis dataKey="village" type="category" tick={{ fontSize: 10 }} width={90} />
                  <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
            ...
            {isLoading ? <Skeleton className="h-52" /> : (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={byMandal} layout="vertical">
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(var(--border))" />
                  <XAxis type="number" tick={{ fontSize: 11 }} />
                  <YAxis dataKey="mandal" type="category" tick={{ fontSize: 10 }} width={90} />
                  <Tooltip contentStyle={{ fontSize: 12, borderRadius: 8 }} />
        ...
        {meetingAttendance.length > 0 && (
          <Card className="p-6">
            <h3 className="text-h3 font-bold mb-4">Public Meeting Attendance</h3>
            <ResponsiveContainer width="100%" height={220}>
        ...
        <Card className="p-6 border-2 border-dashed border-border/60">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Coming soon — AI-powered constituency insights</p>
            </div>
          </div>
          <div className="space-y-2">
            {[
              ...
              <div key={i} className="flex items-center gap-2 rounded-lg bg-muted/40 px-3 py-2 text-xs text-muted-foreground cursor-not-allowed">
                <TrendingUp className="h-3 w-3 shrink-0 text-primary/50" />
                {q}
              </div>
            ))}
          </div>
          <p className="mt-3 text-xs text-muted-foreground">AI features will be available in the next platform update.</p>
        </Card>
      </div>
    </>
  );
