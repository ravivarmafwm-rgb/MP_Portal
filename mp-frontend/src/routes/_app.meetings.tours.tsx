...
function ToursPage() {
  ...
  const totalVillages = tours.reduce((acc, t) => acc + t.villages_count, 0);
  const totalCitizensMet = tours.reduce((acc, t) => acc + t.citizens_met, 0);
    ...
    <>
      ...
      <div className="space-y-6 p-4 md:p-8">
        ...
        {isLoading ? (
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {tours.map((t, i) => {
              const TIcon =
                tourTypeIcon[String(t.tour_type ?? "other")] ?? MapPin;
                  ...
                  <Card className="p-5 flex flex-col gap-3 hover:-translate-y-0.5 hover:shadow-elevated transition-all">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Badge
                        ...
                        className={cn(
                          "shrink-0 text-[10px]",
                          statusTone[String(t.status ?? "planned")],
                        )}
                      >
                        {String(t.status ?? "")}
                      </Badge>
                    ...
                    <div className="space-y-1.5 text-xs text-muted-foreground">
                      <div className="flex items-center gap-1.5">
                        ...
                        {t.end_date &&
                          ` → ${String(t.end_date).substring(0, 10)}`}
                      </div>
                      <div className="flex items-center gap-1.5">
                    ...
                    <div className="grid grid-cols-2 gap-2 rounded-lg bg-muted/40 p-3 text-center text-xs">
                      ...
                      <div>
                        ...
                        >
                          {isPast
                            ? Number(t.citizens_met ?? 0).toLocaleString()
                            : "—"}
                        </div>
      ...
      <Dialog open={showCreate} onOpenChange={setShowCreate}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Plan New Tour</DialogTitle>
          </DialogHeader>
          <div className="grid gap-4">
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setForm((f) => ({ ...f, tour_type: v }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="constituency_visit">
                    Constituency Visit
                  </SelectItem>
                  <SelectItem value="inspection">Inspection</SelectItem>
                  <SelectItem value="project_inspection">
                    Project Inspection
                  </SelectItem>
