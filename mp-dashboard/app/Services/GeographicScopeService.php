...
class GeographicScopeService
{
    ...
    {
        if ($user->hasRole(['super-admin', 'mp']) || (!$user->constituency_id && !$user->assembly_constituency_id && !$user->mandal_id && !$user->village_id && !$user->ward_id)) return $query;
    ...
    {
        if (!$villageId) return !$user->constituency_id && !$user->assembly_constituency_id && !$user->mandal_id && !$user->village_id && !$user->ward_id;
    ...
    public function allowsHierarchicalResource(User $user, Model $model): bool
    {
        ...
        foreach (['constituency_id', 'assembly_constituency_id', 'mandal_id', 'village_id'] as $field) {
            if ($userScope[$field] && $resource[$field] && $userScope[$field] !== $resource[$field]) return false;
        }
    ...
    public function applyHierarchicalResources(Builder $query, User $user): Builder
    {
        ...
        foreach($scope as$field=>$value)if($value)$query->where(fn(Builder $part)=>$part->whereNull($field)->orWhere($field,$value));return$query;
    }
}
